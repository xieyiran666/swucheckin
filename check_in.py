...
def get_token(username: str, password: str, timeout: int = 15):
    ...
    # This will redirect through: of.swu.edu.cn → uaaap.swu.edu.cn → idm.swu.edu.cn
    cas_url = (
    ...
    print("[LOGIN] Starting CAS OAuth flow...")
    try:
        resp = session.get(cas_url, timeout=timeout)
    except requests.exceptions.RequestException as e:
        print(f"[ERROR] CAS initial request failed: {e}")
        return None

    # We should now be at the idm login page (after following redirects)
    final_url = resp.url
    print(f"[LOGIN] Landed at: {final_url[:120]}...")

    # Extract state from the redirect chain (stored in the initial uaaap redirect)
    # The state was in the uaaap URL. We need to find it.
    state = None
    for hist in resp.history:
        if "state=" in hist.url:
            parsed = urllib.parse.urlparse(hist.url)
    ...
    # If we didn't get redirected to idm, try directly
    if "idm.swu.edu.cn" not in final_url:
        print("[LOGIN] Not at idm login page. Trying direct access...")
        # Build the login page URL with service=initService
        login_url = "https://idm.swu.edu.cn/am/UI/Login?service=initService"
        try:
            resp = session.get(login_url, timeout=timeout)
            final_url = resp.url
        except requests.exceptions.RequestException as e:
            print(f"[ERROR] Direct idm access failed: {e}")
            return None
